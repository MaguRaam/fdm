...
void write_matplotlib(const Eigen::VectorXd &x, const Eigen::VectorXd &y, const Eigen::VectorXd &sol)
{
...
void write_vtk(const Eigen::VectorXd &x, const Eigen::VectorXd &y, const Eigen::VectorXd &sol)
{
    int nx = x.size(), ny = y.size();
    auto map = [nx](int i, int j)
    { return nx * j + i; };
